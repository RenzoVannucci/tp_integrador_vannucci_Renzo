...
def index():
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM estudiantes ORDER BY id ASC")
...
def agregar():
    if request.method == "POST":
        ...
        if not dni.isdigit():
            flash("El DNI debe contener solo númeos", "error")
            return redirect(url_for("agregar"))
        
        if email and not re.match(r"^[^@]+@[^@]+\.[^@]+$", email):
            flash("Debe ingresar un email válido.", "error")
            return redirect(url_for("agregar"))
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                ORDER BY apellido ASC, carrera ASC, fecha_registro ASC
                """,
